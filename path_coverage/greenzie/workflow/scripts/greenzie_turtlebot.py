...
def writeFile(status, start_time):
    file = open("/home/ubuntu/greenzie_catkin_ws/src/data/mission_status/run_1", "a")
    if(status == 'failure'):
...
def missionLoop(args, mission_range):
    ...
    for counter in range(mission_range):
        ...
        path_receiver_node = multiprocessing.Process(target = startProcess, args = (['/bin/bash', path_receiver], 'path_receiver_node', start_time, logs))
        path_receiver_node.start()   

        time.sleep(2)
        
        print("Run remote.launch on the remote-pc to launch rosbag and dumpcap. Recording will start when the client is ran.\n")
        ...
        fail_input = input("Press Enter to record mission successful. Press 'f' to record the mission unsuccessful and terminate the processes.\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logs', '-l', type=str, default='/home/ubuntu/greenzie_catkin_ws/src/workflow/logs/turtlebot')
    parser.add_argument('--server', type=str, default='/home/ubuntu/greenzie_catkin_ws/src/workflow/shell_scripts/boustrophedon_server.sh')
    parser.add_argument('--path_receiver', type=str, default='/home/ubuntu/greenzie_catkin_ws/src/workflow/shell_scripts/path_receiver.sh')
    parser.add_argument('--client', type=str, default='/home/ubuntu/greenzie_catkin_ws/src/workflow/shell_scripts/boustrophedon_client.sh')
